-- Fix: remove updated_at = NOW() from trigger (paper_trades has no updated_at column).
-- Also inherit mode/strategy_source from existing open position for orphaned SELL fills.
