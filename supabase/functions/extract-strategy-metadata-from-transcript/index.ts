...
Rules:
- daily_signal: video is about TODAY's trading session — includes "premarket gameplan", "today's watchlist", "stocks for today", market commentary for the day, or gives concrete levels (entry, stop, target) for specific stocks. Even if the creator says "play defensive today" or "no trades today" with no specific levels, it is STILL a daily_signal because it applies only to today and expires tomorrow. Use extracted_signals for any tickers with levels; leave extracted_signals = [] if no specific levels are given. The key distinction: daily_signal = date-specific content. generic_strategy = timeless educational content.
...
- generic_strategy: ONLY for timeless, educational content — general rules, patterns, SMC concepts that apply any day. NOT for daily gameplans, daily watchlists, or any video that references "today", "this morning", "premarket", or a specific date. extracted_signals = [].
