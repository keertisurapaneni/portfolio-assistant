...
| Jun 11 | META short day trade: our P&L -$104.89 vs IB -$107.09; ib_close_order_id null | Bracket stop-loss (BOT order 44191) fired at 19:45:49.458 UTC. Scheduler closed the paper_trade at 19:45:49.309 — 149ms BEFORE the IB fill arrived. `recordTradeClose` found no ib_fills for the order, fell back to formula (-$104.89, no commissions). ib_fill arrived after close, but trigger couldn't match (ib_close_order_id was null). Root cause: bracket SL/TP orders on short day trades are not pre-stamped via `stampAndPlaceClose` (bracket order IDs aren't known at open time). This is a structural gap — bracket fills always race. | DB patch: `b6ae0e75` — set `ib_close_order_id='44191'`, `pnl=-107.09`, `pnl_source='ib_realized'` | none | **Pattern:** any short day trade closed by bracket SL/TP may have wrong P&L if commission report races. IB's commission report (realized_pnl on ib_fills) is the eventual source of truth — if ib_close_order_id is stamped, trigger corrects it. TODO: investigate whether execDetails handler stamps ib_close_order_id for bracket fills. |
