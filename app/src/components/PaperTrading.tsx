...
import { getAccounts } from '../lib/ibClient';
import {
  type PaperTrade,
...
type Tab = 'positions' | 'history' | 'settings';
...
export function PaperTrading() {
  ...
  // Start/stop session ping based on enabled state
  useEffect(() => {
    if (config.enabled) {
      startSessionPing();
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleSync}
              disabled={syncing || !config.accountId}
              className={cn(
        ...
        <div className="flex-1">
          ...
          <p className={cn('text-xs', connected ? 'text-emerald-600' : 'text-red-600')}>
            {connected
              ...
              : 'Login at https://localhost:5000 to connect'}
          </p>
        </div>
        {config.enabled && (
      ...
      <div className="flex gap-1 bg-white/60 p-1 rounded-xl border border-[hsl(var(--border))]">
        {[
          { id: 'positions' as Tab, label: 'Active Positions', icon: Zap, count: activeTrades.length },
