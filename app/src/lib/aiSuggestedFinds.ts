...
function parseCompounderResponse(raw: string): EnhancedSuggestedStock[] {
  ...
  try { parsed = JSON.parse(cleanJSON(raw)); } catch (e) {
    console.warn('[Discovery] Failed to parse compounder JSON:', (e as Error).message, raw.slice(0, 200));
    return [];
  }
...
function parseGoldMineCandidates(raw: string): {
  ...
  let parsed: any;
  try { parsed = JSON.parse(cleanJSON(raw)); } catch (e) {
    console.warn('[Discovery] Failed to parse Gold Mine candidates JSON:', (e as Error).message, raw.slice(0, 200));
    return { theme: { name: 'Market Theme', description: '', categories: [] }, candidates: [] };
  }
...
function parseGoldMineAnalysis(raw: string): EnhancedSuggestedStock[] {
  let parsed: Record<string, unknown>;
  try { parsed = JSON.parse(cleanJSON(raw)); } catch (e) {
    console.warn('[Discovery] Failed to parse Gold Mine analysis JSON:', (e as Error).message, raw.slice(0, 200));
    return [];
  }
  const stocks = parsed.stocks || parsed;
  if (!Array.isArray(stocks)) return [];
...
export async function discoverStocks(
  ...
  // Preserve dipDiscoveries from server cache (generated by auto-trader, not browser)
  let existingDipDiscoveries: EnhancedSuggestedStock[] | undefined;
  try {
    const serverHit = await getServerCachedDiscovery();
    if (serverHit?.data?.dipDiscoveries?.length) {
      existingDipDiscoveries = serverHit.data.dipDiscoveries;
      console.log(`[Discovery] Carrying forward ${existingDipDiscoveries.length} dip discoveries from server cache`);
    }
  } catch { /* best-effort */ }
  ...
  const result: DiscoveryResult = {
    compounders,
    goldMines,
    dipDiscoveries: existingDipDiscoveries,
    currentTheme,
    timestamp: new Date().toISOString(),
  };
